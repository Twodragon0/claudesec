#!/usr/bin/env python3
"""
ClaudeSec Dashboard — Overview HTML builders.

Extracted from dashboard_html_sections.py via the Option B split documented in
.omc/plans/dashboard-standards-split.md.  Owns the three Overview-tab
builders: the service-surface coverage grid, the priority-queue action
cards, and the aggregate ``_build_overview_blocks`` orchestrator that the
main dashboard generator calls.
"""
...
def build_service_surface_html(
    findings_list,
    total_checks,
    failed,
    warnings,
...
def build_overview_blocks(
    ...
    env_total = len(envs)
    service_surface_html = build_service_surface_html(
        findings_list,
        total_checks,
        failed,
        warnings,
    ...
    priority_queue_html = build_priority_queue_html(
        findings_list,
        prov_summary,
        env_connected,
        env_total,
    ...
    config_html, net_enabled, net_targets, trivy_enabled = build_network_config_section()
    network_tools_html = config_html

    network_tools_html += build_tooling_readiness_section(net_data, net_enabled, net_targets, trivy_enabled)
