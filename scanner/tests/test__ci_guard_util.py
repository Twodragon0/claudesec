...
class TestShellIfBodyCommandPosition(unittest.TestCase):
    """PoCs from the adversarial review of #402. Each anchored the scan inside a
    non-shell region and yielded a body containing `exit 1` while the real gate
    was deleted and bash exited 0."""

    _ANCHOR = r'\[\s*"\$CRITS"\s+-gt\s+0\s*\]\s*;\s*then'
    _LIVE = (
        '          if [ "$CRITS" -gt 0 ]; then\n'
        '            echo "::error::critical findings"\n'
        "            exit 1\n"
        "          fi\n"
    )

    def test_live_gate_is_found(self):
        body = shell_if_body(self._LIVE, self._ANCHOR)
        self.assertIsNotNone(body)
        self.assertIn("exit 1", body)

    def _assert_decoy_caught(self, decoy, why):
        body = shell_if_body(decoy, self._ANCHOR) or ""
        self.assertNotIn("exit 1", body, why)

    def test_anchor_inside_a_single_quoted_string_does_not_count(self):
        self._assert_decoy_caught(
            """          echo 'legacy: if [ "$CRITS" -gt 0 ]; then exit 1; fi'\n          echo ok\n""",
            "a quoted decoy anchored the scan inside itself — the gate can be "
            "deleted while the guard certifies it intact",
        )

    def test_anchor_inside_a_heredoc_body_does_not_count(self):
        self._assert_decoy_caught(
            "          cat <<'NOTE'\n"
            '          if [ "$CRITS" -gt 0 ]; then exit 1; fi\n'
            "NOTE\n          echo ok\n",
            "a heredoc body anchored the scan; heredoc bodies must be blanked",
        )

    def test_anchor_inside_a_yaml_scalar_does_not_count(self):
        self._assert_decoy_caught(
            '      - name: legacy if [ "$CRITS" -gt 0 ]; then exit 1; fi\n',
            "a YAML scalar anchored the scan; the anchor must start a line",
        )

    def test_deleted_gate_returns_none(self):
        self.assertIsNone(shell_if_body('          echo "$CRITS recorded"\n', self._ANCHOR))

    def test_anchor_that_already_carries_if_still_matches(self):
        # The image-size gate's anchor includes `if`; the severity gate's does
        # not. Both must resolve, hence the optional leading `if`.
        text = '          if [ "$size_mb" -gt "$max_mb" ]; then\n            exit 1\n          fi\n'
        body = shell_if_body(text, r'if\s*\[\s*"\$size_mb"\s+-gt\s+"\$max_mb"\s*\]\s*;\s*then')
        self.assertIsNotNone(body)
        self.assertIn("exit 1", body)


class TestBlankHeredocBodies(unittest.TestCase):
    def test_body_is_blanked_and_line_count_preserved(self):
        src = "a\ncat <<'EOF'\nsecret\nEOF\nb\n"
        out = blank_heredoc_bodies(src)
    ...
    def test_unquoted_and_dash_forms(self):
        for opener in ("<<EOF", "<<-EOF", '<<"EOF"'):
            out = blank_heredoc_bodies(f"cat {opener}\nsecret\nEOF\n")
            self.assertNotIn("secret", out, opener)

    def test_text_without_heredoc_is_unchanged(self):
        src = "echo one\necho two\n"
        self.assertEqual(blank_heredoc_bodies(src), src.rstrip("\n"))
