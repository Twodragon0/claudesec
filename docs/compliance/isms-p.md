...
## 미평가(N/A) 정책 — 3.x 개인정보 통제

일부 통제는 동의·처리방침·정보주체 권리·국외이전 계약처럼 **법적·거버넌스 성격**이라
기술 스캐너가 자동으로 판정할 수 없습니다. NIST SP 800-53A는 통제 평가를 자동화 가능한
기술 점검뿐 아니라 문서 검토(Examine)·인터뷰(Interview)·테스트(Test)로 구분하며,
이런 절차·문서 통제는 심사원의 수동 검토(Examine/Interview) 대상입니다
([NIST SP 800-53A Rev.5, Assessment Methods](https://csrc.nist.gov/pubs/sp/800/53/a/r5/final)).

ClaudeSec는 이런 통제를 **PASS로 위장하지 않습니다.** 파인딩이 없다고 자동으로 초록(PASS)을
칠하면 감사 맥락에서 **허위 준수 보증(false compliance assurance)** 이 되기 때문입니다.
대신 `assessable: false`로 태깅하여 대시보드에 **N/A(미평가, 수동 검토 필요)** 로 렌더하고,
PASS/FAIL 집계에서 제외합니다 (`total = pass + fail`, N/A는 별도 카운트).

### N/A로 렌더되는 11개 통제 (파일럿 범위: ISMS-P 3.x 개인정보 처리단계)
...
- N/A 통제는 키워드가 매칭돼도 **FAIL로 뒤집히지 않습니다.** 매칭된 파인딩은
  informational count로만 기록됩니다 (`scanner/tests/test_compliance_map_ismsp_na.py`가 강제).
- 파일럿 범위는 **ISMS-P 3.x 개인정보 통제로 한정**됩니다. 다른 프레임워크(NIST/ISO/PCI/CIS)의
  거버넌스·절차 통제로의 확대는 후속 작업으로 분리되어 있습니다.

> ISMS-P 인증에서 3.x 개인정보 통제는 심사원의 문서 심사·인터뷰로 확인됩니다
> ([ISMS-P 인증기준 안내서 2023.11](https://isms.kisa.or.kr/main/ispims/notice/)).
