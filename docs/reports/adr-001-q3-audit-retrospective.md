...
description: "ADR-001 §3 반복 적대 검증으로 CI 가드 파싱 검출기의 잔여 우회를 찾은 #297 분기 감사 회고 — 5-패스 체인, Class-1/2 구분, 양방향 정규식, 무력 단정의 메타 가드화, 스택 PR 트리거 갭"
tags:
  - session-report
  - security
  - ci-cd
...
ADR-001 §3의 "1차가 CRITICAL을 찾으면 2차 패스"를 이번엔 **관점이 다른 두 리뷰어
