...
> 다음 발견을 만들고, 그 발견이 **회고 자체를 반증했다.** 네 번 — 세 번째는 이
> 문서를(2.4절), 네 번째는 **그 세 번째가 만든 산출물을**(2.5절) 반증했다.
...
| 후속 | 08-11 | [#422](https://github.com/Twodragon0/claudesec/pull/422) `b6655b6` | 규칙을 ADR §9·체크리스트·스킬에 등재 | 2038 passed |
...
## 2. 중심 구조 — 체크리스트가 자기 회고를 네 번 반증했다

PR #409는 회고 문서 하나와, `guard-audit-reminder.yml`의 분기 체크리스트에 들어갈
규칙 몇 줄을 남겼다(#410에서 실제 등록). 그 규칙이 처음 돌자마자 **자기를 만든
회고의 두 절을 무효화**했다. 2.4절은 세 번째이며 반증 대상이 이 문서이고, 2.5절은
네 번째이며 반증 대상이 2.4절의 산출물이다 — 사슬은 아직 돌고 있다.
...
### 2.5 반증 4 — 2.4절이 만든 산출물이 하루 만에 네 군데서 틀렸다 (#420~#425)

2.4절은 "블록 추출기를 고쳤고, 규모가 되는 해법은 다섯 개의 패치가 아니라 **열거**"
라고 끝났다. 그 열거와 그 규칙들을 실제로 등재·검증하는 후속 사이클이 곧바로 **네
군데의 반증**을 냈다. 이번엔 반증 대상이 남의 문서도, 이 문서도 아니라 **직전 절이
막 만들어낸 산출물**이다.

| 반증 | 2.4절이 남긴 것 | 실제 |
|---|---|---|
| [#420](https://github.com/Twodragon0/claudesec/pull/420) | 수집기 1건을 "무영향(no observable effect)"으로 판정 | **판정 자체가 틀렸다.** 수집기는 주석에 취약하고, 가드는 `setUpClass`의 `strip_comment_lines` 한 줄 때문에 면역이었다 — 면역이 **호출자**에 있었고 아무도 단정하지 않았다 |
| [#425](https://github.com/Twodragon0/claudesec/pull/425) | 열거표를 카탈로그에 기록 | **그 표가 이미 불완전했다.** 소스에서 파생해 diff하자마자 `test_ci_injection_surface.run_block_lines`가 빠져 있었다(`_lead_width`로 탭을 확장해 들여쓰기를 재므로 감사의 눈에 안 들어왔다) |
| [#423](https://github.com/Twodragon0/claudesec/pull/423) | 규칙을 ADR §9로 등재([#422](https://github.com/Twodragon0/claudesec/pull/422)) | **인용하려던 그 문서의 번호가 어긋나 있었다.** `ADR-001 §4` 인용 26건 중 **23건이 §5**를 뜻했다. 등재라는 행위가 피인용 문서의 무결성을 반증했다 |
| [#424](https://github.com/Twodragon0/claudesec/pull/424) | "매핑에서는 주석이 아무것도 끝내지 않는다" | **"strip 먼저"가 보편 규칙이 아님이 드러났다.** folded scalar에서 `#` 줄은 **내용**이므로 pre-strip은 살아있는 CLI 토큰을 지운다. ADR §1/§2가 요구하는 순서가 여기서는 틀리다 |

#### 이 반증들이 앞의 셋과 다른 점

2.1·2.2절은 **체크리스트가 회고를 반증**했고, 2.4절은 **회고가 자기를 반증**했다.
2.5절은 **검증 장치를 만드는 행위 자체가 그 장치의 입력을 반증**했다. 세 경우 모두
같은 메커니즘이지만, 마지막 것이 가장 값싸다 — 파생·diff는 사람의 재검토를 기다리지
않는다. #425는 작성 당일에 누락 1건을 냈고, #423의 번호 고정 가드는 앞으로의 삽입을
빌드 실패로 만든다.

#### 규칙이 아니라 **비**규칙도 나온다

이번 사이클에서 가장 값진 결과는 수정이 아니라 **부정 결과 둘**이다.

1. **"항상 strip 먼저"는 문법에 따라 갈린다**(#424). 매핑에서는 맞고 스칼라에서는
   틀리다. 그래서 "raw 텍스트를 추출기에 넘기지 말라"는 기계적 검출기를 **추가하지
   않았다** — 같은 스위트에 정당한 반례가 있고, 예외 목록이 발견보다 긴 가드는
   sprawl이다. 두 예외는 각자 자리의 테스트로 고정했다.
2. **일괄 치환은 두 번 잘못된 답이었다**(#423, [#426](https://github.com/Twodragon0/claudesec/pull/426)).
   §4 인용 26건 중 2건은 §4가 맞았고, `sed` 한 방은 그 2건을 반대 방향으로 틀리게
   만든다. 인용은 **번호가 아니라 인용된 텍스트**로 분류해야 한다 — 한 번은
   토픽으로 추측했다가 `lighthouse_perf_gate`의 "inert-assertion class"를 §4로 옮길
   뻔했다(정답은 §2).

   다만 이 절이 함께 적어 둔 "§2·§3 22건 중 4건은 두 결정이 모두 맞아 **보류가
   정답**"은 **틀렸다**. 텍스트가 동점일 때 판정하는 것은 텍스트가 아니라 **출처**다
   → 3절 표 마지막 행.

#### 프로브가 세 번 더 틀렸다

3·4절의 "하네스를 먼저 공격하라"가 이 사이클에서 세 번 재현됐다. 전부 **가드가 아니라
프로브의 오류**였고, 셋 다 모순이 신호였다.

- 잡 블록 **시작**에 주석을 넣으면 `steps:`까지 잘려 "잡혔다"로 읽힌다 → 배치를 끝으로
  옮겨야 진짜 우회가 드러난다(#419).
- 수집기에 **raw** 텍스트를 직접 넘겨 `None`을 얻고 "우회"라고 판단했으나, 같은 프로브의
  `16 passed`와 모순됐다 → 답은 `setUpClass`에 있었다(#420).
- folded scalar 안 key 컬럼에 주석을 심고 "오탐 3건"으로 판정했으나, 그 형태는 애초에
  `ParserError`다 → 유효하지 않은 뮤테이션은 아무것도 증명하지 않는다(#424).

#### 운영에서도 같은 클래스가 나왔다

[#427](https://github.com/Twodragon0/claudesec/pull/427)은 가드가 아니라 CI 운영
쪽이지만 형태가 같다. `Lint` 런이 `completed success`인데 체크런 3개가 `in_progress`로
남아 필수 체크가 확정되지 않았다 — **실패가 아니라 정체**여서 "실패한 워크플로는 2회까지
재실행" 규칙이 걸리지 않고, `gh pr checks`로는 평범한 도커 빌드와 구별되지 않는다.
판정을 세 조건의 **논리곱**(체크런 미완 ∧ 런 완료 ∧ 임계 시간 초과)으로 정의해야
건강한 작업을 재실행하지 않는다. 기본은 report-only, 재실행은 opt-in.

---

## 3. 문서도 통제다 — 그리고 문서는 썩는다

이 사이클은 **문서 부패**를 네 번 서로 다른 형태로 잡았다. 가드 코드가 아니라
문서가 결함의 소재지였던 사례들이다.
...
| [#426](https://github.com/Twodragon0/claudesec/pull/426) → [#430](https://github.com/Twodragon0/claudesec/pull/430) | "§2 인용 4건은 두 결정이 모두 맞으니 **보류가 정답**"이 2.5절과 7절에 **규칙**으로 등재 | 4건 전부 **§3**(Two-pass adversarial review)이었다 — 넷 다 `b1fb276`(PR #379, 07-30)에서 왔고 그날의 §2가 오늘의 §3, `9a530d6`(PR #390)이 4h37m 뒤 새 §2를 끼워넣어 밀었다 | 보류가 **영구 면허**가 될 뻔했다 — "두 해석 모두 가능"은 백로그에 안 남고 재조사되지도 않는다 |
...
ADR-001 §5("열거에 대한 세 번째 패치는 재설계 신호")가 이 사이클에서 다섯 번
적용됐다.
...
12. **인용은 번호가 아니라 인용된 텍스트로 분류하고, 텍스트가 동점이면 도입 커밋으로
    분류한다.** 일괄 치환은 잘못된 답이었다 — §4 26건 중 2건은 §4가 맞았다(#423).
    반대로 "두 결정이 모두 맞으니 보류"도 답이 아니었다: 그렇게 남긴 §2 인용 4건은
    `git blame`이 전부 **§3**으로 확정했다(#426 → #430). 쓰여진 날의 §2가
    오늘의 §3이고, `9a530d6`(#390)의 §2 삽입이 4h37m 만에 밀어버린 것이다 —
    **인용은 쓰여진 날의 번호 체계만큼만 안정하다.** 번호가 붙은 목록에는 항목을
    **끼워넣지 않는다**; 끝에 추가하고, 번호↔제목을 가드로 고정한다.
13. **자동화는 report-only가 기본, 상태를 바꾸는 동작은 opt-in.** 그리고 판정은 조건의
    논리곱으로 정의한다 — 조건 하나하나는 정상 상태와 구별되지 않는다(#427).
