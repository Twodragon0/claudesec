...
2. **일괄 치환은 두 번 잘못된 답이었다**(#423, [#426](https://github.com/Twodragon0/claudesec/pull/426)).
   §4 인용 26건 중 2건은 §4가 맞았고, §2·§3 인용 22건 중 4건은 두 결정이 모두
   들어맞아 **손대지 않는 것이 정답**이었다. `sed` 한 방은 맞는 것을 반대 방향으로
   틀리게 만든다. 인용은 **번호가 아니라 인용된 텍스트**로 분류해야 한다 — 한 번은
...
12. **인용은 번호가 아니라 인용된 텍스트로 분류한다.** 일괄 치환은 두 번 잘못된 답이었다
    — §4 26건 중 2건은 §4가 맞았고, §2·§3 22건 중 4건은 두 결정이 모두 맞아 손대지
    않는 것이 정답이었다(#423, #426). 번호가 붙은 목록에는 항목을 **끼워넣지 않는다**;
    끝에 추가하고, 번호↔제목을 가드로 고정한다.
