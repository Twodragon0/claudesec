...
**KNOWN GAP, opened 2026-09-07 — the residual canary does not fire on the PR
shape that would introduce a residual.**
...
Closing it means adding the catalog to the `scanner` bucket, which fires kcov
(minutes) — the cheap alternative is a third job pinned to
`requirements-ci.txt`. Left open with the tradeoff stated rather than papered
over. The guard-suite coverage invariant is NOT affected: the three inventory
guards read no Markdown at all.
