...
| `scanner/tests/test_ci_slash_command_sync.py` | The slash commands README's `Claude Code slash commands` block advertises are exactly the tracked `.claude/commands/*.md` files | direction is EQUALITY with the FILESYSTEM authoritative: a command file is what Claude Code loads and the README restates it, so advertising a name with no file fails AND shipping a command nobody lists fails. Neither direction is the safe one — an unlisted command is undiscoverable, a listed non-existent one is a broken instruction. **The drift was real and unguarded** — measured 2026-09-21 on `main`, the block listed seven names of which FOUR (`/prowler`, `/compliance`, `/audit`, `/team-scan`) had no definition anywhere, while five that exist (`/compliance-check`, `/hotfix`, `/new-guide`, `/pipeline-review`, `/scanner-feature`) were unlisted. `/prowler` and `/compliance` are the trap: `grep` finds both in `package.json`, which reads like a definition and is actually the npm KEYWORDS array — the CLI really does have `npx claudesec prowler`, so the names are real but not as slash commands, which is exactly the confusion a list under that heading creates. **The Markdown reduction is used in the opposite direction from the other doc guards**: `rendered_markdown` strips fenced code among its four evasions and this list LIVES in a fence, so reducing before searching would delete the subject and make every run read as "block missing". The two steps are split instead — the section MARKER is checked against the REDUCED text (so a heading hidden in a closed comment or after an unterminated `<!--` fails loudly rather than silently narrowing scope to nothing) and the names are read from the RAW fence (where they are, and where a browser shows them). Checking the marker against raw text would let someone comment out the section and keep this green over invisible content; checking the names against reduced text would find nothing, ever. Scoped to the one section (ADR-001 §2) because `npx claudesec scan` and other command-shaped tokens appear elsewhere and an unscoped search invents commands — pinned by `test_prose_outside_the_block_is_not_collected`. The command set comes from `tracked_files()`, never a filesystem glob, so an untracked local `.claude/commands/scratch.md` cannot turn CI red-locally/green-in-CI. Six inverse mutations including both hiding shapes, plus a positive control on the extractor (`>= 4` real names) without which every absence assertion would be satisfiable by an extractor that returns nothing. stdlib-only, no `scanner/lib` import | this PR |
