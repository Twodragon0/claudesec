...
| `scanner/tests/test_ci_action_pin_labels.py` | INTERNAL CONSISTENCY of the version LABEL on a SHA-pinned `uses:` ref | One `(action, sha)` carries at most one label (A); one `(action, label)` maps to at most one sha (B); an action labelled at ANY site is labelled at EVERY site (C). C is what gives A and B reach — with six bare `setup-python` refs and one label there was nothing to contradict, which is how the drift survived. Found live 2026-09-17: `lint.yml` pinned `actions/setup-python@5fda3b95…  # v6.1.0` while that SHA is **v7.0.0** (the real `v6.1.0` is `83679a892e2d`, eight months older) — the SHA is what runs, so the cost is a reader or next bumper reasoning one MAJOR below reality; it is the incident #552 quotes as its deliberately-wrong example, found in a live workflow. **What it CANNOT do, so its green is not over-read:** checking a label against the REAL tag needs the GitHub API, and guards are offline and stdlib-only — that belongs in a scheduled watch workflow (`protection-drift-watch` pattern) and is NOT built. Re-measured against `origin/main` pre-fix: the detector reports both mixed-label actions. Mutation self-tests plant each of A, B and C and assert a DELTA against the clean control rather than an empty list (#553), drive the real `label_problems` rather than a surrogate (#536), and pin the out-of-scope set (tag-pinned, `./`, `docker://`) plus the commented-out-ref false-alarm direction. Subpaths share one identity so a `codeql-action/init` vs `/analyze` disagreement is reported — the #478/#483/#511/#549 four-site shape | this PR |
