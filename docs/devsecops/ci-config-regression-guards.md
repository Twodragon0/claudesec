...
- **Shared primitives**: comment-stripping, continuation-joining, and `on:`-block
  ...
  `action.yml`). **Never write `key\s*:` by hand** — compose `yaml_key_pattern`. Import them as a
...
| `scanner/tests/test_ci_security_gate.py` | `Security Scan Gate` + DAST signals | `security-scan-gate` keeps `name`, `if: always()`, `needs: ⊇ {changes, scan, lighthouse}`, pass-set `not in (success, skipped)`; `dast-baseline.yml` keeps its `pull_request` trigger; `dast-full-scan.yml` keeps its `schedule:` trigger AND a `coverage-notice` job that opens an issue when `DAST_TARGET_URL` is unset — the trigger alone is not enough, since the scan job is gated on that variable and 42 consecutive nightly runs reported a clean `skipped` (six weeks of ZERO automated DAST, no red build, workflow name still promising nightly coverage). Comment-stripped scan, so a `#`-parked notice job does not satisfy it. The CRITICAL severity block is isolated with the shared `shell_if_body` primitive after the 2026-08-07 audit found two bypasses of the local copy, both PoC'd on bash AND on the guard: the anchor was searched in RAW text (one comment line `# was: if [ "$CRITS" -gt 0 ]; then exit 1; fi` anchored the scan inside itself and yielded ` exit 1; ` as the body), and an unbalanced if/fi count returned the rest of the FILE, so commenting the block out line-by-line plus any later `exit 1` read green. Now strips first and fails CLOSED | #205, #216, #397, this PR **Not fully closed — read the gap before trusting this row.** #402 stripped `#` comments before searching the anchor, but the adversarial review of #402 re-opened the identical evasion three more ways (quoted string, heredoc body, YAML scalar), each yielding a body of `exit 1;` scraped from the decoy while the real block was deleted and bash exited 0. Those are now closed by line-anchoring the search and blanking heredoc bodies (`_ci_guard_util.blank_heredoc_bodies`). **KNOWN GAP, direction UNDER-report i.e. a real bypass:** a syntactically present but non-executing control still satisfies the scan — `if false; then <gate> fi`, an `elif` arm that cannot be true, and `case … never) exit 1 ;;` all give bash `exit=0` on a CRITICAL with the guard GREEN. Deciding reachability is interpretation, not scanning; the only line-scanner rule that would catch them would fire on every legitimately nested conditional. Revisit trigger: if any lands in a real workflow this needs a bash parser. |
...
| `scanner/tests/test_ci_docker_image_size_gate.py` | Docker image-size cap in `lint.yml` (`dashboard-regression-check`) | the `max_mb=N` size gate is present, breaches the build (`size_mb -gt max_mb` → `exit 1`), and `N <= 600` MB — locking in the ~513 MB scanner image achieved by prowler-provider stripping (was 1.47 GB, cycle #217-#237, issue #11). Loosening the cap back toward the old 1.8 GB or re-adding a stripped ~700 MB provider would slip through; tightening DOWN stays green (CIS Docker Benchmark — minimal image surface). The `exit 1` assertion is scoped to the breach `if … fi` block by balanced depth — unscoped it was satisfied by any of `lint.yml`'s dozen unrelated `exit 1` lines and could not detect removal of this gate's own exit (inert-guard class). Comment-evasion-proof for BOTH the `max_mb` cap and the `exit 1` scan: the whole scan routes through the bash-aware `strip_inline_comment_sh`, so the `;#exit 1` metachar adjacency #383 recorded as a KNOWN OPEN under-strip is closed (primitive from #376). The block extractor now delegates to the shared `shell_if_body`: its local copy counted `\bif\b` ANYWHERE and fell back to "the rest of the file" when the count never balanced, which was a LIVE false negative — adding the English word "if" to the breach message ("… check the base image if this is unexpected") and deleting the `exit 1` returned a 9546-char body carrying an unrelated prowler `exit 1`, so bash let a 900 MB image through (verified) while this guard reported the gate intact. `if` is now counted only in bash command position and an unbalanced block fails CLOSED | #11, this PR |
...
- **HIGH (latent), same root cause in `conditional_body_from`** — commenting the
  ...
  Both are now the shared `_ci_guard_util.shell_if_body`: strips first, counts
  `if` only in bash command position, and fails CLOSED when unbalanced.
