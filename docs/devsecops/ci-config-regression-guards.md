...
| `scanner/tests/test_ci_security_gate.py` | `Security Scan Gate` + DAST signals | `security-scan-gate` keeps `name`, `if: always()`, `needs: ⊇ {changes, scan, lighthouse}`, pass-set `not in (success, skipped)`; `dast-baseline.yml` keeps its `pull_request` trigger; `dast-full-scan.yml` keeps its `schedule:` trigger AND a `coverage-notice` job that opens an issue when `DAST_TARGET_URL` is unset — the trigger alone is not enough, since the scan job is gated on that variable and 42 consecutive nightly runs reported a clean `skipped` (six weeks of ZERO automated DAST, no red build, workflow name still promising nightly coverage). Comment-stripped scan, so a `#`-parked notice job does not satisfy it | #205, #216, #397 **Finding #1 from the 2026-08-07 audit is NOT fixed — do not read this row as covering it.** The anchor is still searched in RAW text, so one line `# was: if [ "$CRITS" -gt 0 ]; then exit 1; fi` anchors the scan inside itself, returns ` exit 1; ` as the body, and certifies a DELETED gate while bash exits 0 on a CRITICAL — behind the REQUIRED `Security Scan Gate`. Three successive line-scanner fixes were each defeated by adversarial review (4 shapes, then 6: quoted string, heredoc body, multi-line YAML scalar, space-indented heredoc terminator, two heredocs on a line, split/non-word delimiters), which is ADR-001's "a third patch to an enumeration is a redesign signal". The replacement is BEHAVIOURAL, not a better parser: extract the gate's `run:` block and EXECUTE it against a CRITICAL fixture, asserting a non-zero exit — a prototype closes every shape above AND the three reachability shapes no line scanner can reach (`if false; then`, an unreachable `elif`, `case … never) exit 1`). Tracked in `test_ci_strip_before_match.KNOWN_STRIP_ORDER_EXCEPTIONS`. |
...
- **HIGH (latent), same root cause in `conditional_body_from`** — commenting the
  ...
  **Finding #1 is confirmed but NOT fixed** — see its catalog row. Three
  line-scanner fixes were each defeated; the replacement is a behavioural
  test that executes the gate. What DID land is the docker-size half. It counts
