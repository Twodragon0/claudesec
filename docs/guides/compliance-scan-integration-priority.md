...
- **작업**:
  ...
  - 필요 시 `docs/compliance/soc2.md` 추가. (출처: AICPA TSC, [security-citations.mdc](.cursor/rules/security-citations.mdc) 준수.)
...
4. **보안 인용**: 컴플라이언스 가이드·대시보드 설명은 [security-citations.mdc](.cursor/rules/security-citations.mdc)에 따라 OWASP/NIST/CIS/공식 표준 문서를 인용.
5. **민감 정보 금지**: [no-sensitive-paths.mdc](.cursor/rules/no-sensitive-paths.mdc) 준수. 예제에는 실제 경로·계정 ID·고객사명을 넣지 않음.
