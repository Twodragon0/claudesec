...
- `claudesec-dashboard.html` (the security-scan dashboard; no longer served —
  its compliance/OWASP rollups are published into `scan-report.json` and
  rendered inside the ISMS dashboard, and `/scan.html` 301s to `/`) plus
